...
inline fun <reified T : RealmObject> T.update() {
    this.save()
}

inline fun <reified T : RealmObject> T.updateByKey(primaryKey: Any, crossinline updateFunctor: (T) -> Unit) {
 ...
 * */
inline fun <reified T : RealmObject> T.update(findFunctor: RealmFinder.() -> Unit, updateFunctor: (T) -> Unit) {
    this.findAll { findFunctor() }?.forEach {
        updateFunctor(it)
    }
}